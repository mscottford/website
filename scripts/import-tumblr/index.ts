...
function extractImages(
  ...
  updatedHtml = updatedHtml.replace(
    /<span[^>]*class="[^"]*tmblr-alt-text-helper[^"]*"[^>]*>ALT<\/span>/gi,
    '',
  )
...
function convertPost(
  ...
  switch (post.type) {
    ...
    case 'link':
      ...
      description =
        decodeHtmlEntities(linkDesc.replace(/<[^>]+>/g, '').trim()).substring(
          0,
          157,
        ) || `Link to ${linkUrl}`
      break
    ...
    case 'conversation':
      title =
        decodeHtmlEntities(post['conversation-title'] || '') || 'Conversation'
  ...
  if (!description) {
    description =
      ...
      `${title} - A blog post from ${date.getFullYear()}`
  }
