...
const sortedImagesAndMetadata = imagesAndMetadata.sort((a, b) => {
...
const groupedIcons = Object.groupBy(
  sortedImagesAndMetadata,
  (item: FaviconAndMetadata) =>
...
await Promise.all(
  sortedImagesAndMetadata.map(async (image) => {
    const destinations = [path.join(dest, image.icon.name)]
